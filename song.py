...
class songAPI:
    ...
    async def queue(self, ctx):
        ...
        if voice_client == None or not voice_client.is_connected():
                embed = discord.Embed(title="❌Error to see queue", description="Bot is not in channel", color=0xFF0000)
                await ctx.send(embed=embed)
                return
            
        player = self.get_player(ctx)
        if player.queue.empty():
                embed = discord.Embed(title="queueList", description="quequ is empy", color=0xFFFFFF)
                await ctx.send(embed=embed)
                return
        ...
        embed = discord.Embed(title=f'queueList-({len(upcoming)})', description=fmt, color=0x1a91ff)
        await ctx.send(embed=embed)  
